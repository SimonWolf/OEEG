...
def download_day_long(standort: str, date_str: str,ttl_hash=None) -> pd.DataFrame:
    ...
    raw_df.columns = ["Datetime"] + [
        f"WR{i + 1}" for i in range(len(raw_df.columns) - 1)
    ]
    ...
    for wr_col in raw_df.columns[1:]:
        ...
        melted = split_df.reset_index().melt(
            id_vars="index", var_name="col_idx", value_name="value"
        )
        ...
        if not wr_idx_match:
            raise ParseError(
                f"Konnte WR-Index nicht aus Spaltennamen '{wr_col}' extrahieren."
            )
    ...
    if not long_parts:
        # defensive: falls keine WR-Spalten gefunden wurden
        raise ParseError(
            "Keine Wechselrichter-Spalten verarbeitet (keine Teile erzeugt)."
        )
...
def get_heutige_Leistung(standort: str) -> np.ndarray:
    ...
    try:
        df = get_day_and_update(standort, today)    
        
        df_filtered = (
            df[(df["string"] == -1) & (df["sensor"] == "P") & (df["Datetime"].dt.date == today)]
            .groupby("Datetime", as_index=False)["value"]
            .sum()
            .rename(columns={"value": "P_gesamt"})
