...
def plot_calendar_heatmap(
    ...
    if weekday_locale is None:
        weekday_locale = locale_name
    days = _weekday_labels(rows=rows, locale_name=weekday_locale, fmt=weekday_format)
        ...
        while month_start <= end_date:
            ...
            if 0 <= wi < cols:
                x_ticktext[wi] = (
                    ...
                    if month_abbrev
                    else calendar.month_name[month_start.month]
                )
            month_start = (
    ...
    holes = [
        _rounded_path(j - 0.5, i - 0.5, j + 0.5, i + 0.5, r=corner_ratio)
        for i in range(rows)
        for j in range(cols)
    ]
